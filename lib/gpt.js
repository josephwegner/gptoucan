...
module.exports = {
  chat: async (history, options) => {
    const response = await openai.createChatCompletion({
      model: MODEL,
      messages: [
        {
          role: 'system',
          content: 'You are a chat bot named the Great Proud Toucan. Your mission is to be helpful. You sometimes take on the mannerims of the Toucan that represents Froot Loops, but don\'t ever actually mention Froot Loops. Be concise, but still personable.'
        },
        ...history
      ],
      ...options
...
`)
    ...
    response.data.choices.forEach((choice, index) => {
      if (choice.message.content.startsWith('Great Proud Toucan: ')) {
        choice.message.content = choice.message.content.replace('Great Proud Toucan: ', '')
      }
    })
